...
public class CDMapsActivity extends FragmentActivity implements OnMapReadyCallback,
    ...
    private CheckDamResponse checkDamResponse;
    double distance = 0.0;
    String fromClass = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            String otData = sharedPreferences.getString("CD_DATA", "");
            fromClass = sharedPreferences.getString("FROM_CLASS", "");
            try {
                if (fromClass.equalsIgnoreCase("DASH")) {
                    switchView.setText("SHOW LIST");

                } else if (fromClass.equalsIgnoreCase("CD")) {
                    switchView.setText("CheckDam DETAILS");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }

            checkDamResponse = gson.fromJson(otData, CheckDamResponse.class);
    ...
    private void setMarkers(final CheckDamResponse checkDamResponse) {
        try {
            ...
            MapAdaptor customInfoWindow = new MapAdaptor(CDMapsActivity.this);
            mMap.setInfoWindowAdapter((GoogleMap.InfoWindowAdapter) customInfoWindow);

            if (fromClass.equalsIgnoreCase("CD")) {
            ...
            public void onResult(LocationSettingsResult result) {
                ...
                switch (status.getStatusCode()) {
                    case LocationSettingsStatusCodes.SUCCESS:
                        if (ConnectionDetector.isConnectedToInternet(CDMapsActivity.this)) {
                            SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                                    .findFragmentById(R.id.map);
                            mapFragment.getMapAsync(CDMapsActivity.this);


                        } else {
                            AlertDialog.Builder alert = new AlertDialog.Builder(CDMapsActivity.this);
                    ...
                    case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                        try {
                            status.startResolutionForResult(CDMapsActivity.this, REQUEST_LOCATION_TURN_ON);
                        } catch (IntentSender.SendIntentException e) {
                            Toast.makeText(CDMapsActivity.this, "Something Went Wrong", Toast.LENGTH_SHORT).show();
                        }
                        break;
    ...
    private boolean requestPermissions() {
        ...
        if (ContextCompat.checkSelfPermission(CDMapsActivity.this,
                Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(CDMapsActivity.this,
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_LOCATION_TURN_ON) {
            if (resultCode == RESULT_OK) {
                ...
                new AlertDialog.Builder(CDMapsActivity.this).setCancelable(false).setTitle("Please turn on the location to continue").setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        turnOnLocation();
